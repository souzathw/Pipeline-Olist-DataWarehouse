...
with DAG(
    dag_id="olist_load_redshift_sellers",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=["olist", "redshift", "staging", "sellers"],
) as dag:
    ...
    def load(ingestion_date: str) -> dict:
        ...
        s3_path = f"s3://{bucket}/bronze/olist/sellers/ingestion_date={ingestion_date}/"
        ...
        hook.run(sql)
        rows = hook.get_first("select count(*) from stg.sellers;")[0]
