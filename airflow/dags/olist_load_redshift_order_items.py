...
with DAG(
    dag_id="olist_load_redshift_order_items",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=["olist", "redshift", "staging", "order_items"],
) as dag:
    ...
    def load(ingestion_date: str) -> dict:
        ...
        s3_path = f"s3://{bucket}/bronze/olist/order_items/ingestion_date={ingestion_date}/"

        sql = f"""
        truncate table stg.order_items;

        copy stg.order_items (
          order_id,
          order_item_id,
          product_id,
          seller_id,
        ...
        rows = hook.get_first("select count(*) from stg.order_items;")[0]
