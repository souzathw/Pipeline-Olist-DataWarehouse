...
with DAG(
    dag_id="olist_load_redshift_product_category_translation",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=["olist", "redshift", "staging", "products"],
) as dag:
    ...
    def load(ingestion_date: str) -> dict:
        ...
        s3_path = f"s3://{bucket}/bronze/olist/product_category_translation/ingestion_date={ingestion_date}/"

        sql = f"""
        truncate table stg.product_category_translation;

        copy stg.product_category_translation (
        ...
        update stg.product_category_translation
        ...
        rows = hook.get_first("select count(*) from stg.product_category_translation;")[0]
